...
if(!instance.isQuietingDown()) {
    // Read properties
    def properties = new ConfigSlurper().parse(new File("/var/jenkins_home/jenkins.properties").toURI().toURL())
    ...
    properties.credentials.each {
        ...
        switch (it.value.type) {
            case "ssh":
                ...
                creds = new BasicSSHUserPrivateKey(CredentialsScope.GLOBAL, it.value.credentialsId, it.value.userId, new BasicSSHUserPrivateKey.FileOnMasterPrivateKeySource(it.value.path), it.value.passphrase, it.value.description)
            ...
            case "password":
                ...
                creds = new UsernamePasswordCredentialsImpl(CredentialsScope.GLOBAL, it.value.credentialsId, it.value.description, it.value.userId, new File(it.value.path).text.trim())
                credentials_store.addCredentials(global_domain, creds)
                break
            default:
